...
const ProfileImg = styled.div`
  ...
  > label {
    ...
    font-size: 30px;
    > span {font-size: var(--fz-base); padding-top: 10px; line-height: 1.2;}
  }
...
const DefaultProfile = styled.div`
  ...
  background: var(--bl-1);
  font-size: 60px; color: var(--mainbl);
`;
      ...
      {editMode ? (
        <ProfileWrap>
          <ProfileImg className={image.preview_URL ? 'uploaded' : null}>
            <label htmlFor='imageUpload'><BsFillImageFill/><span>클릭해서 업로드</span></label>
            <input
              type="file"
              accept="image/*"
              id='imageUpload'
            ...
            />
            {image.preview_URL?<img src={image.preview_URL} alt='프로필이미지' />:null}
        ...
        <ProfileWrap>
          <ProfileImg>
            {image.preview_URL?<img src={image.preview_URL} alt='내 프로필' />:<DefaultProfile><BsFillImageFill/></DefaultProfile>}
          </ProfileImg>
          <ProfileContent>
            <ProfileItem>
              <li>
